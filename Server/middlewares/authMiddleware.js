...
export const isAuthenticated = catchAsyncErrors(async (req, res, next) => {
  ...
  //kyuki jwt sign krte wqt token nam ediya tha isliye yha bhi token nam ka variable use kiya hai

  //token req ki cookie m pada hoga
  ...
  next(); //error middlee]ware call hoga next() se
});

//req.user m se roles lekr aaye and check kiye ki aagr wo roles match ho rha ya nai aagr nai toh return error 403 means unauthorized access
...
export const authorizedRoles = (...roles) => {
  return (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new ErrorHandler(
          `Role: ${req.user.role} is not access this resource`,
          403,
        ),
      );
    }
