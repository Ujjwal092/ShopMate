...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  if (password.length < 8 || password.length > 16) {
    return next(
      new ErrorHandler("Password should be in between 8 to 16 chars", 400),
    );
  }
  ...
  const isAlreadyRegistered = await database.query(
    ...
    [email],
    //$1 means whatever is just after comma and [email] is array
  );
  ...
  //RETURNING * means return the newly created user record after insertion
...
export const login = catchAsyncErrors(async (req, res, next) => {
  ...
  //req.user.password nai chl ra hai kyuki wo auth middleware m set hota hai aur login m auth middleware use nai ho rha hai isliye user.rows[0].password se compare kr rhe hai
...
export const getUser = catchAsyncErrors(async (req, res, next) => {
  // const { user } = req;
  const user = req.user;
  //this is coming from auth middleware where we set req.user = user.rows[0] after verifying the token
...
export const logout = catchAsyncErrors(async (req, res, next) => {
  res
    ...
    .cookie("token", "", {
      ...
      secure: true, //  REQUIRED for http (localhost)
    })
    .json({
      success: true,
      message: "Logged Out",
...
export const updatePassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!currentPassword || !newPassword || !confirmNewPassword) {
    return next(
      new ErrorHandler("All fields are required to update password", 400),
    );
  }
  ...
  if (!isPasswordMatch) {
    return next(
      new ErrorHandler("Password is not matching with original password", 401),
    );
    //401 forbidden
  }
  ...
  if (newPassword !== confirmNewPassword) {
    return next(
      new ErrorHandler("New password doesn't match with confirm new password"),
      400,
    );
  }
