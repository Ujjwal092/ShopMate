      ...
      <div className="flex items-start space-x-4">
        <div className="flex-1">
          ...
          <div className="flex items-center mt-2">
            {Array.from({
              length: Math.min(5, Math.max(0, t.rating || 0)),
            }).map((_, i) => (
          ...
          {t.product_id && (
            <Link
              to={`/products?product=${t.product_id}`}
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-2">
            <button onClick={prev} className="p-2 glass-card">
              <ChevronLeft />
            </button>
            <button onClick={next} className="p-2 glass-card">
              <ChevronRight />
            </button>
          </div>
        </div>
        ...
        <div className="flex justify-center space-x-2 mt-4">
          {testimonials.map((_, i) => (
            <button
              key={i}
              onClick={() => setIndex(i)}
              className={`w-3 h-3 rounded-full ${i === index ? "bg-primary" : "bg-white/30"}`}
            ></button>
          ))}
        </div>
      </div>
